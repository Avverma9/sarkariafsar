...
const SECTION_NAME = 'Recent Admit Cards'
const SECTION_SLUG = 'admit-cards'
const COLOR = { bg: 'bg-purple-600', light: 'bg-purple-50', text: 'text-purple-700', border: 'border-purple-200', icon: '🪪' }
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors">
      <td className="px-4 py-2.5">
        <Link href={`/jobs/${job.slug}`} className="text-purple-700 hover:underline font-medium leading-snug line-clamp-2">
          {job.title}
        </Link>
      </td>
      ...
      <div className="bg-[#1e3a5f] text-white py-14 px-4">
        <div className="container mx-auto max-w-5xl">
          <nav className="text-sm text-blue-300 mb-3" aria-label="breadcrumb">
            ...
            <span className="text-white">Admit Cards</span>
          </nav>
          <h1 className="text-3xl font-bold">Sarkari Admit Card 2026</h1>
          <p className="text-blue-200 mt-2">Download hall tickets for latest government exams</p>
        </div>
      </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {jobs.length === 0
                  ? <tr><td colSpan={3} className="py-20 text-center text-gray-400">No admit cards found</td></tr>
