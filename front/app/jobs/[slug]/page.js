...
// ─────────────────────────────────────────────────────────────────────────────
// BUG FIX 1: Shared fetch function — avoids double API call
//   Previously: generateMetadata + JobDetailPage each called fetch separately
//   with cache:‘no-store’, meaning 2 independent network requests per page load.
//   If either failed, data was lost silently.
//
// BUG FIX 2: res.ok check — avoids crash on 4xx/5xx responses
//   Previously: fetch could return a 500/404 and .json() would still be called,
//   throwing a parse error caught silently.
//
// BUG FIX 3: Replaced cache:‘no-store’ with next:{revalidate:60}
//   Previously: Every request bypassed Next.js cache entirely.
//   Now: Data is cached for 60 seconds, shared between generateMetadata and
//   the page component — one fetch instead of two.
// ─────────────────────────────────────────────────────────────────────────────
async function getJobData(slug) {
try {
const res = await fetch(`${API_BASE}/post/slug/${slug}`, {
next: { revalidate: 60 }, // shared cache — both generateMetadata & page reuse this
})
if (!res.ok) return null          // BUG FIX 2: don’t call .json() on error responses
...
// BUG FIX 4: Resources fetch also now uses revalidate instead of no-store
async function getResourceData(jobId) {
try {
const res = await fetch(`${API_BASE}/resource/by-post/${jobId}`, {
next: { revalidate: 120 },
})
...
// BUG FIX 1: Uses shared getJobData() — Next.js deduplicates this fetch
// with the one in JobDetailPage when revalidate values match
...
return { title: ‘Job Details - Sarkari Afsar’ }
}
}
...
// BUG FIX 5: Broken emoji unicode escapes fixed — using actual emoji characters
// Previously: `'📚'` style broken strings were present in resource typeConfig
...
const href = resource.accessType === ‘external’ ? resource.url : resource.fileUrl
return (
<a key={i} href={href}
...
// ─────────────────────────────────────────────────────────────────────────────
// MAIN PAGE
// BUG FIX 1 (applied): getJobData() is now called once — Next.js fetch
// deduplication ensures generateMetadata and JobDetailPage share the same
// cached response when revalidate values match (both 60s).
// ─────────────────────────────────────────────────────────────────────────────
export default async function JobDetailPage({ params }) {
const { slug } = await params

// BUG FIX 1: Single shared fetch — no more double network request
const job = await getJobData(slug)
if (!job) return notFound()

// BUG FIX 4: Resources also use revalidate now
const resources = await getResourceData(job._id)
