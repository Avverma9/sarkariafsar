...
export default function SchemeDetailPage({ scheme }) {
  const hasOverview = Boolean(scheme?.about);
  const hasProcess = Array.isArray(scheme?.process) && scheme.process.length > 0;
  const hasDocuments = Array.isArray(scheme?.documents) && scheme.documents.length > 0;
