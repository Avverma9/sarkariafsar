    ...
    <div className="bg-gray-50 min-h-screen">
      <div className="bg-[#1e3a5f] text-white py-10 px-4">
        <div className="container mx-auto max-w-3xl">
          <h1 className="text-2xl font-bold mb-4">🔍 Search Sarkari Afsar</h1>
        ...
        {!loading && searched && (
          <>
            ...
            {results.jobs.length > 0 && (
              <div className="mb-8">
                <h2 className="text-base font-bold text-[#1e3a5f] mb-3">💼 Jobs ({results.jobs.length})</h2>
                <div className="space-y-2">{results.jobs.map(j => <SearchResultItem key={j._id || j.slug} item={j} type="job" />)}</div>
              </div>
            )}
            {results.schemes.length > 0 && (
              <div className="mb-8">
                <h2 className="text-base font-bold text-[#1e3a5f] mb-3">🏛️ Schemes ({results.schemes.length})</h2>
                <div className="space-y-2">{results.schemes.map(s => <SearchResultItem key={s._id || s.slug} item={s} type="scheme" />)}</div>
              </div>
            )}
            {results.blogs.length > 0 && (
              <div className="mb-8">
                <h2 className="text-base font-bold text-[#1e3a5f] mb-3">📝 Blog ({results.blogs.length})</h2>
