...
export function getSectionConfig(sectionSlug) {
  ...
  if (["newjobs", "latestjobs", "jobs", "newjob", "latestjob"].includes(normalized)) {
    return {
...
export async function generateMetadata({ params }) {
  ...
  if (!config) {
    return buildPageMetadata({
      ...
      description: "Requested section not found.",
      path: "/post",
      noIndex: true,
    });
  }
...
export default async function PostSectionPage({ params, searchParams }) {
  const resolvedParams = await params;
  const config = getSectionConfig(resolvedParams?.section);
  ...
  return (
    <PostPageShell>
      <SectionJobsPage basePath={`/post/${config.canonicalSlug}`} {...pageData} />
    </PostPageShell>
  );
}
